 ...
 public:
  ...
  std::array<uint8_t, kSha256Length> Finalize(std::string_view chunk,
    ...
    *next = uint8_t{0x80};
    ...
    // 64-bit block with the length of the message in big-endian order. If
    // chunk.size() + 9 > 64, pad out to the next 512-bit block.
    if (chunk.size() + 9 > 64) {
      Update(std::string_view(padding_chunk.data(), padding_chunk.size()));
      std::fill(padding_chunk.begin(), padding_chunk.begin() + 56, 0);
    }

    padding_chunk[56] = (original_size >> 56) & 0xffu;
    padding_chunk[57] = (original_size >> 48) & 0xffu;
    padding_chunk[58] = (original_size >> 40) & 0xffu;
    padding_chunk[59] = (original_size >> 32) & 0xffu;
    padding_chunk[60] = (original_size >> 24) & 0xffu;
    padding_chunk[61] = (original_size >> 16) & 0xffu;
    padding_chunk[62] = (original_size >> 8) & 0xffu;
    padding_chunk[63] = original_size & 0xffu;
    Update(std::string_view(padding_chunk.data(), padding_chunk.size()));
...
std::array<uint8_t, kSha256Length> Sha256(std::string_view str) {
  ...
  return hasher.Finalize(str, original_size);
}
