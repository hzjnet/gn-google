...
void Printer::PrintTokensWrapped(const std::vector<Token>& comments) {
  ...
  for (const auto& c : comments) {
    ...
    bool is_indented =
        c.value().length() >= 3 && c.value().substr(0, 3) == "#  ";
    bool has_url = trimmed.find("http://") != std::string::npos ||
                   trimmed.find("https://") != std::string::npos;
    bool is_list =
        trimmed.length() >= 4 && trimmed[0] == '#' && trimmed[1] == ' ' &&
        (trimmed[2] == '-' || trimmed[2] == '*' || isdigit(trimmed[2])) &&
        trimmed[3] == ' ';
    bool is_copyright =
        trimmed.find("Copyright") != std::string::npos ||
    ...
    bool is_attention_marker =
        text_only.find("TODO") == 0 || text_only.find("NOTE") == 0 ||
        text_only.find("WARNING") == 0 || text_only.find("FIXME") == 0;
    ...
    bool is_special = is_indented || has_url || is_empty || is_list ||
                      is_copyright || is_pragma || is_single_word ||
