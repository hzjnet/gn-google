...
class Printer {
 ...
 private:
  ...
  void PrintCommentsWrapped(const std::vector<Token>& comments);
...
// or statement.
void Printer::PrintCommentsWrapped(const std::vector<Token>& comments) {
...
void Printer::PrintSuffixComments(const ParseNode* node) {
  if (node->comments() && !node->comments()->suffix().empty()) {
    ...
    PrintCommentsWrapped(node->comments()->suffix());
    stack_.pop_back();
  }
}
...
void Printer::FlushComments() {
  if (!comments_.empty()) {
    ...
    stack_.push_back(IndentState(CurrentColumn(), false, false));
    PrintCommentsWrapped(comments_);
    stack_.pop_back();
    comments_.clear();
  }
}
