...
class Target : public Item {
 public:
  ...
  // Calls OnResolvedWithoutChecks followed by RunChecksAfterResolution
  ...
  // standard dependencies (i.e. without validations and gen_deps) considered
  // final and immutable. This allows running checks in background checks while
  // the rest of the resolution algorithm continues in the main thread.
  bool OnResolvedWithoutChecks(Err* err);

  // Run checks. This requires OnResolvedWithoutChecks() to have been called
  // first. This can rrun in a background thread.
