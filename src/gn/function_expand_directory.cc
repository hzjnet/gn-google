...
Value ExpandDirectoryInternal(const ParseNode* function,
  ...
  g_scheduler->AddGenDependency(disk_path.StripTrailingSeparators());
  ...
  for (base::FilePath current = traverser.Next(); !current.empty();
    ...
    if (traverser.GetInfo().IsDirectory()) {
      g_scheduler->AddGenDependency(current.StripTrailingSeparators());
    } else {
      std::string full = source_path.value() + FilePathToUTF8(current).substr(disk_path_utf8.size());
      NormalizePath(&full);
