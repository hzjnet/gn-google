...
Value ExpandDirectoryInternal(const ParseNode* function,
  ...
  for (base::FilePath current = traverser.Next(); !current.empty();
    ...
    if (traverser.GetInfo().IsDirectory()) {
      ...
      std::string rel_path;
      // This should have been checked in RunExpandDirectory already.
      DCHECK(MakeAbsolutePathRelativeIfPossible(
...
Value RunExpandDirectory(Scope* scope,
  ...
  std::string dummy_rel_path;
  if (!MakeAbsolutePathRelativeIfPossible(root_path, FilePathToUTF8(dir_path),
                                          &dummy_rel_path)) {
    *err = Err(function, "Directory is outside the source root.");
    return Value();
  }
  ...
  Value result =
      ExpandDirectoryInternal(function, root_path, dir_path, recursive);
