...
TEST_F(HeaderCheckerTest, IsDependencyOf) {
  ...
  auto& a_cache = checker->GetCacheForTarget(&a_);
  EXPECT_FALSE(checker->IsDependencyOf(&a_, &a_, a_cache, &chain, &is_permitted));

  // A depends publicly on B.
  chain.clear();
  is_permitted = false;
  EXPECT_TRUE(checker->IsDependencyOf(&b_, &a_, a_cache, &chain, &is_permitted));
  ...
  chain.clear();
  is_permitted = false;
  EXPECT_TRUE(checker->IsDependencyOf(&c_, &a_, a_cache, &chain, &is_permitted));
  ...
  chain.clear();
  is_permitted = false;
  auto& c_cache = checker->GetCacheForTarget(&c_);
  EXPECT_FALSE(checker->IsDependencyOf(&a_, &c_, c_cache, &chain, &is_permitted));
  ...
  auto& a_cache2 = checker->GetCacheForTarget(&a_);
  EXPECT_TRUE(checker->IsDependencyOf(&c_, &a_, a_cache2, &chain, &is_permitted));
  ...
  chain.clear();
  is_permitted = false;
  auto& p_cache = checker->GetCacheForTarget(&p);
  EXPECT_TRUE(checker->IsDependencyOf(&c_, &p, p_cache, &chain, &is_permitted));
  ASSERT_EQ(2u, chain.size());
  EXPECT_EQ(HeaderChecker::ChainLink(&c_, false), chain[0]);
  EXPECT_EQ(HeaderChecker::ChainLink(&p, true), chain[1]);
  EXPECT_TRUE(is_permitted);
...
TEST_F(HeaderCheckerTest, CheckInclude) {
  ...
  auto& a_cache = checker->GetCacheForTarget(&a_);
  checker->CheckInclude(&a_, a_cache, input_file, d_header, range, &errors);
  EXPECT_GT(errors.size(), 0);

  // A can include the public header in B.
  errors.clear();
  checker->CheckInclude(&a_, a_cache, input_file, b_public, range, &errors);
  EXPECT_EQ(errors.size(), 0);

  // Check A depending on the public and private headers in C.
  errors.clear();
  checker->CheckInclude(&a_, a_cache, input_file, c_public, range, &errors);
  EXPECT_EQ(errors.size(), 0);
  errors.clear();
  checker->CheckInclude(&a_, a_cache, input_file, c_private, range, &errors);
  EXPECT_GT(errors.size(), 0);

  // A can depend on a random file unknown to the build.
  errors.clear();
  checker->CheckInclude(&a_, a_cache, input_file, SourceFile("//random.h"), range,
                        &errors);
  EXPECT_EQ(errors.size(), 0);
  ...
  // dependency path.
  errors.clear();
  checker->CheckInclude(&a_, a_cache, input_file, otc_header, range, &errors);
  EXPECT_EQ(errors.size(), 0);
}
...
TEST_F(HeaderCheckerTest, PublicFirst) {
  ...
  bool is_permitted = false;
  HeaderChecker::Chain chain;
  EXPECT_TRUE(checker->IsDependencyOf(&d_, &a_, a_cache, &chain, &is_permitted));
  ...
  auto& a_cache2 = checker->GetCacheForTarget(&a_);
  chain.clear();
  EXPECT_TRUE(checker->IsDependencyOf(&d_, &a_, a_cache2, &chain, &is_permitted));
...
TEST_F(HeaderCheckerTest, CheckIncludeAllowCircular) {
  ...
  auto checker = CreateChecker();
  auto& b_cache = checker->GetCacheForTarget(&b_);

  // A depends on B. So B normally can't include headers from A.
  std::vector<Err> errors;
  checker->CheckInclude(&b_, b_cache, input_file, a_public, range, &errors);
  EXPECT_GT(errors.size(), 0);
...
TEST_F(HeaderCheckerTest, CheckIncludeSwiftModule) {
  ...
  auto checker = CreateChecker();
  auto& d_cache = checker->GetCacheForTarget(&d_);
  ...
  errors.clear();
  checker->CheckInclude(&d_, d_cache, input_file, generated_header, range,
                        &errors);
  EXPECT_GT(errors.size(), 0);

  // Check that unrelated target D cannot include S's bridge header.
  errors.clear();
  checker->CheckInclude(&d_, d_cache, input_file, bridge_header, range, &errors);
  EXPECT_GT(errors.size(), 0);
}
...
TEST_F(HeaderCheckerTest, Friend) {
  ...
  std::vector<Err> errors;
  checker->CheckInclude(&b_, b_cache, input_file, c_private, range, &errors);
  EXPECT_GT(errors.size(), 0);
