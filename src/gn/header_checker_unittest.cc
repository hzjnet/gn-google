...
TEST_F(HeaderCheckerTest, IsDependencyOf) {
  ...
  EXPECT_FALSE(
      checker->IsDependencyOf(&a_, a_cache, &chain, &is_permitted));

  // A depends publicly on B.
  chain.clear();
  is_permitted = false;
  EXPECT_TRUE(
      checker->IsDependencyOf(&b_, a_cache, &chain, &is_permitted));
  ...
  is_permitted = false;
  EXPECT_TRUE(
      checker->IsDependencyOf(&c_, a_cache, &chain, &is_permitted));
  ...
  EXPECT_FALSE(
      checker->IsDependencyOf(&a_, c_cache, &chain, &is_permitted));
  ...
  EXPECT_TRUE(
      checker->IsDependencyOf(&c_, a_cache2, &chain, &is_permitted));
...
TEST_F(HeaderCheckerTest, CheckInclude) {
  ...
  errors.clear();
  checker->CheckInclude(a_cache, input_file, SourceFile("//random.h"),
                        range, &errors);
  EXPECT_EQ(errors.size(), 0);
...
TEST_F(HeaderCheckerTest, PublicFirst) {
  ...
  HeaderChecker::Chain chain;
  EXPECT_TRUE(
      checker->IsDependencyOf(&d_, a_cache, &chain, &is_permitted));
  ...
  chain.clear();
  EXPECT_TRUE(
      checker->IsDependencyOf(&d_, a_cache2, &chain, &is_permitted));
...
TEST_F(HeaderCheckerTest, CheckIncludeSwiftModule) {
  ...
  errors.clear();
  checker->CheckInclude(d_cache, input_file, generated_header, range,
                        &errors);
  EXPECT_GT(errors.size(), 0);

  // Check that unrelated target D cannot include S's bridge header.
  errors.clear();
  checker->CheckInclude(d_cache, input_file, bridge_header, range,
                        &errors);
  EXPECT_GT(errors.size(), 0);
}
