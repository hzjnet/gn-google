...
TEST(NinjaTargetWriter, ValidationsWithNoOutput) {
  ...
  // Should contain the normal dep.
  EXPECT_NE(std::string::npos, out.find("phony/foo/dep"));
}

TEST(NinjaTargetWriter, GroupWithValidations) {
  TestWithScope setup;
  setup.build_settings()->set_no_stamp_files(true);
  Err err;
  ...
  // This should NOT crash, and should write a phony rule.
  writer.WriteStampOrPhonyForTarget(deps, order_only);

  std::string out = stream.str();
  EXPECT_NE(std::string::npos,
            out.find("build phony/foo/target: phony |@ phony/foo/val"));
}
