...
TEST_F(BinaryTargetGeneratorTest, AllowCircularIncludesAllowlist) {
  ...
  Err err;
  dep->SetToolchain(setup.toolchain(), &err);
  dep->OnResolved(&err);
  items_.push_back(std::unique_ptr<Item>(dep));

  // Case 1: Allowlist not set. Allowed.
  {
    TestParseInput input(
        R"(static_library("foo") {
             sources = [ "//foo.cc" ]
  ...
  // Case 2: Allowlist set, file IN list. Allowed.
  {
  ...
  // Case 3: Allowlist set, file NOT in list. Rejected.
  {
    auto allowlist = std::make_unique<SourceFileSet>();
