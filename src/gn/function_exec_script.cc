...
#if defined(OS_WIN)
    // A relative `interpreter_path` can be interpreted as being relative to
    // the build directory only on Windows.  See the comment in
    // `BuildSettings::SetPythonPath`.
    //
    // On other platforms, the `interpreter_path` is assumed to come from the
    // `PATH` environment variable.
    if (!interpreter_path.IsAbsolute()) {
