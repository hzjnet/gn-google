...
Options
  ...
  --tree
    ...
    R"(
  --file_relation=(source|public|input|data|script|output)
...
int RunRefs(const std::vector<std::string>& args) {
  ...
  for (const std::string& relation : cmdline->GetSwitchValueStrings("file_relation")){
    if (relation == "source"){
      ...
      Err(Location(), "Unknown file_relation: " + relation).PrintToStdout();
      return 1;
    }
  }
