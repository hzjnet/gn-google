...
bool HeaderChecker::CheckFile(const Target* from_target,
  ...
  ReachabilityCache& from_target_cache = GetReachabilityCacheForTarget(from_target);
  ...
  while (iter.GetNextIncludeString(&include)) {
    ...
    if (!included_file.is_null()) {
      CheckInclude(from_target_cache, input_file, included_file, include.location, errors);
    }
  }
...
HeaderChecker::ReachabilityCache& HeaderChecker::GetReachabilityCacheForTarget(
  ...
  if (it == shard.cache.end()) {
    it = shard.cache
             .emplace(target, std::make_unique<ReachabilityCache>(target))
             .first;
  }
  return *it->second;
}
...
bool HeaderChecker::IsDependencyOf(const Target* search_for,
                                   ...
                                   Chain* chain) const {
  return from_target_cache.SearchForDependencyTo(search_for,
                                                 require_permitted,
                                                 chain);
}
