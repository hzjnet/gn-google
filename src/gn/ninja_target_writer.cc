...
std::string NinjaTargetWriter::RunAndWriteFile(
  ...
  if (target->output_type() == Target::BUNDLE_DATA) {
    ...
    if (target->module_type() == Target::GENERATED_TEXTUAL_MODULEMAP) {
      SourceDir out_dir =
          GetBuildDirForTargetAsOutputFile(target, BuildDirType::GEN)
              .AsSourceDir(settings->build_settings());
      StringOutputBuffer modulemap_storage;
      std::ostream os(&modulemap_storage);
      writer.WriteModuleMap(os, out_dir);

      base::FilePath file_path =
          settings->build_settings()->GetFullPath(out_dir).AppendASCII(
              target->label().name() + ".modulemap");
