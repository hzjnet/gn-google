...
TEST_F(SetupTest, AllowBuildDirSymlinks) {
  ...
  base::FilePath source_root =
      in_path.AppendASCII(FILE_PATH_LITERAL("source_root"));
  CreateDirectory(source_root);
  base::FilePath dot_gn_name = source_root.Append(FILE_PATH_LITERAL(".gn"));
  WriteFile(dot_gn_name, kDotfileContents);

  WriteFile(source_root.Append(FILE_PATH_LITERAL("BUILDCONFIG.gn")), "");
  cmdline.AppendSwitchPath(switches::kRoot, source_root);

  WriteFile(source_root.Append(FILE_PATH_LITERAL("build_defines.gni")),
            "variable1 = true");
