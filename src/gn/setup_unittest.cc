...
TEST_F(SetupTest, ArgsGnRelativeImport) {
  base::CommandLine cmdline(base::CommandLine::NO_PROGRAM);
...
script_executable = ""
  ...
  WriteFile(args_gn_name, R"(import("parameters.gni"))");
  base::FilePath parameters_gni_name =
      build_temp_dir.GetPath().Append(FILE_PATH_LITERAL("parameters.gni"));
  WriteFile(parameters_gni_name, R"(variable = true)");

  // Run setup and check that the parameters.gni file is in dependency files.
  Setup setup;
  Err err;
  EXPECT_TRUE(setup.DoSetupWithErr(FilePathToUTF8(build_temp_dir.GetPath()),
                                   true, cmdline, &err));
  ...
  ASSERT_EQ(1u, dependency_files.size());
  EXPECT_EQ(dependency_files.begin()->GetName(), "parameters.gni");
}
