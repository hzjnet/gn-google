...
script_executable = ""
  ...
  char16_t original_cwd[MAX_PATH];
  ::GetCurrentDirectory(MAX_PATH, reinterpret_cast<LPWSTR>(original_cwd));
  ...
  ::SetCurrentDirectory(
      reinterpret_cast<LPCWSTR>(cwd_temp_dir.GetPath().value().c_str()));
#endif
  ...
  ::SetCurrentDirectory(reinterpret_cast<LPCWSTR>(original_cwd));
#else
