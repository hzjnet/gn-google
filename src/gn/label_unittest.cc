...
TEST(Label, GetUserVisibleName) {
  ...
  for (const auto& test_case : test_cases) {
    ...
    EXPECT_EQ(test_case.expected,
        ...
        << "dir: " << test_case.dir.value() << " name: " << test_case.name;
  }

  // Also test the null label case.
...
TEST(Label, GetUserVisibleNameDefaultToolchain) {
  const struct TestCase {
    ...
    const char* expected;
      ...
      {SourceDir("//dir/"), "name", SourceDir("//t/"), "tn", SourceDir("//t/"),
       "tn", "//dir:name"},
      // Different toolchain (dir doesn't match), so toolchain is printed.
      {SourceDir("//dir/"), "name", SourceDir("//t/"), "tn",
       SourceDir("//other/"), "tn", "//dir:name(//t:tn)"},
      // Different toolchain (name doesn't match), so toolchain is printed.
      {SourceDir("//dir/"), "name", SourceDir("//t/"), "tn", SourceDir("//t/"),
       "other", "//dir:name(//t:tn)"},
      // Different toolchain (label's toolchain dir is null), so toolchain is
      // printed.
      {SourceDir("//dir/"), "name", SourceDir(), "tn", SourceDir("//t/"), "tn",
       "//dir:name()"},
      // Different toolchain (default toolchain dir is null), so toolchain is
      // printed.
      {SourceDir("//dir/"), "name", SourceDir("//t/"), "tn", SourceDir(), "tn",
       "//dir:name(//t:tn)"},
      // Label dir is null hence label is null.
      {SourceDir(), "name", SourceDir("//t/"), "tn", SourceDir("//t/"), "tn",
       ""},
  };
